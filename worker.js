// Filename: worker.js (UPDATED with progress reporting)
...
const replacements = {
    ' & ': ' and ', ' % ': ' percent ', ' per cent ': ' percent ', ' honourable ': ' hon ', ' honble ': ' hon ', ' hon\'ble ': ' hon ',
    ' doctor ': ' dr ', ' mister ': ' mr ', ' misses ': ' mrs ', ' governments ': ' govts ', ' government ': ' govt ',
    ' special ': ' spl ', ' through ': ' thru ', ' department ': ' dept ', ' association ': ' assoc ', ' first ': ' 1st ',
    ' second ': ' 2nd ', ' third ': ' 3rd ', ' rupees ': ' rs ', ' rupee ': ' re ', ' advertisement ': ' advt ',
    ' additional ': ' addl ', ' secretary ': ' secy ', ' versus ': ' vs ', ' limited ': ' ltd ', ' private ': ' pvt ',
    ' number ': ' no ', ' ma\'am ': ' madam '
};
...
function levenshtein(a, b) {
    ...
    if (!a || !b) return levenshteinCache[key] = (a || "").length + (b || "").length;
...
function analyzeText(originalText, typedText, timeTakenSeconds, options = {}) {
    // NEW: Send the first progress update
    self.postMessage({ type: 'progress', message: 'Preparing texts...' });
    ...
    for (let i = 1; i <= normOriginalWords.length; i++) {
        // NEW: Send an update every 25 words
        if (i % 25 === 0) {
            self.postMessage({ type: 'progress', message: `Comparing word ${i} of ${normOriginalWords.length}...` });
        }
    ...
    // NEW: Send another progress update
    self.postMessage({ type: 'progress', message: 'Aligning results...' });
    ...
    // NEW: Send final progress update
    self.postMessage({ type: 'progress', message: 'Calculating final score...' });
    ...
    return {
        alignment: alignment, fullMistakes: fullMistakes, halfMistakes: halfMistakes,
        accuracy: accuracy.toFixed(2), mistakeScore: mistakeScore,
        mistakePercent: (totalWords > 0 ? (mistakeScore / totalWords * 100) : 0).toFixed(2),
        totalWords: totalWords, typedWords: typedWordCount, typingSpeed: typingSpeed.toFixed(2),
...
self.onmessage = function(event) {
    ...
    // 2. Run the heavy, slow calculation (which will now send progress messages)
    const results = analyzeText(originalText, typedText, timeTakenSeconds, options);
    
    // 3. Send the FINAL finished result back to the main script
    // NEW: The final message is wrapped in an object with a 'result' type
